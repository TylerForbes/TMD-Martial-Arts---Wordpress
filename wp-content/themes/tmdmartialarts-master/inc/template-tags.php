 ...
 */
  ...
  */
 function TMD_Martial_Arts_posted_on() {
  ...
  */
 function TMD_Martial_Arts_posted_by() {
 	$byline = sprintf(
 		esc_html( 'by %s' ),
  ...
  */
 function TMD_Martial_Arts_comment_count() {
  ...
  */
 function TMD_Martial_Arts_entry_footer() {
 	// Hide category and tag text for pages.
 	if ( 'post' === get_post_type() ) {
 		/* translators: used between list items, there is a space after the comma */
 		$categories_list = get_the_category_list( esc_html( ', ' ) );
 		if ( $categories_list && TMD_Martial_Arts_categorized_blog() ) {
  ...
  * @return bool
  */
 function TMD_Martial_Arts_categorized_blog() {
 	if ( false === ( $all_the_cool_cats = get_transient( 'TMD_Martial_Arts_categories' ) ) ) {
 		...
 		set_transient( 'TMD_Martial_Arts_categories', $all_the_cool_cats );
 	}

 	if ( $all_the_cool_cats > 1 ) {
 		// This blog has more than 1 category so TMD_Martial_Arts_categorized_blog should return true.
 		return true;
 	} else {
 		// This blog has only 1 category so TMD_Martial_Arts_categorized_blog should return false.
 		return false;
 	}
 }

 /**
  * Flush out the transients used in TMD_Martial_Arts_categorized_blog.
  */
 function TMD_Martial_Arts_category_transient_flusher() {
 	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
 		return;
 	}
 	// Like, beat it. Dig?
 	delete_transient( 'TMD_Martial_Arts_categories' );
 }
 add_action( 'edit_category', 'TMD_Martial_Arts_category_transient_flusher' );
 add_action( 'save_post',     'TMD_Martial_Arts_category_transient_flusher' );
  ...
  */
 function TMD_Martial_Arts_comment_list( $comment, $args, $depth ) {
  ...
  */
 function TMD_Martial_Arts_numbered_pagination() {
 	global $wp_query;
 	$big = 999999999;
